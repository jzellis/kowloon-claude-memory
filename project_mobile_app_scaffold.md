...
- **Video posts don't start playing until fully loaded.** Server-side cause — uploaded MP4s have their `moov` atom at the end (the default), so the player can't start streaming until it has the whole file. Fix is to remux uploaded videos to faststart MP4 (`moov` at the front) in the file-upload pipeline — needs `ffmpeg` on the server, not built yet. `expo-video` has no client-side flag that helps; this is purely a server-side encoding concern.
